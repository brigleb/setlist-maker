...
def analyze_loudness(audio_file: Path) -> dict | None:
    """
    Analyze the loudness of an audio file using FFmpeg loudnorm filter.

    Args:
        audio_file: Path to the audio file.

    Returns:
        Dictionary with loudness statistics, or None if analysis fails.
    ...
    try:
        ...
        for line in output.split("\n"):
            ...
            if "Input Integrated:" in line:
                stats["input_i"] = line.split(":")[-1].strip().split()[0]
            elif "Input True Peak:" in line:
                stats["input_tp"] = line.split(":")[-1].strip().split()[0]
            elif "Input LRA:" in line:
                stats["input_lra"] = line.split(":")[-1].strip().split()[0]
            elif "Input Threshold:" in line:
                stats["input_thresh"] = line.split(":")[-1].strip().split()[0]
